...
class FundaListingParser
  ...
  class << self
    ...
    def address(slop)
      slop.at_css('a.object-street').content.strip rescue ''
    end
    ...
    def price(slop)
      slop.at_css('span.price-wrapper span.price').content rescue ''
    end
    
    def visiting(slop)
      (slop.css('span.nvm-open-huizen-dag span') || []).map { |el| el.content rescue '' }.join(', ')
    end
    
    def since(slop)
      slop.at_css('span.item-since').content rescue ''
    end
